...
{
    public partial class SetInterview : System.Web.UI.Page
    {
        ...
        public static List<DataRow> candidateList = new List<DataRow>();
        public static List<DataRow> selectedCandidateList = new List<DataRow>();
        public List<DataRow> candidateStatusList = new List<DataRow>();
        public static Dictionary<string, string> intCandidates;
        public Dictionary<string, string> leftPanelStatusList = Constants.qualifiedCandidateStatus;
        ...
        private void GetQualifiedCandidates(int job_id)
        {
            try
            {
                var url = string.Format("api/Jobs/GetQualifiedsList?job_id=" + job_id + "&status=" + Request.QueryString["status"]);
                DataTable dt = wHelper.GetDataTableFromWebApi(url);
        ...
        private static void GetSelectedCandidates(int interview_id)
        {
            try
            {
        ...
        public static int UpdateCandidates(Dictionary<string, Dictionary<string, string>> candidatesDetails, string interview_id, string status)
        {
            ...
            //Response.Redirect(ConfigurationManager.AppSettings["WebURL"].ToString());
            //GetQualifiedCandidates(Convert.ToInt32(HttpContext.Current.Request.QueryString["job_id"]));
            GetSelectedCandidates(Convert.ToInt32(HttpContext.Current.Request.QueryString["id"]));
            return ret;
        }
        ...
        public int UpdateCandidatesForInterview()
        {
            int ret = 0;
